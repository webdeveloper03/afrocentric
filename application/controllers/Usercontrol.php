...
class Usercontrol extends MY_Controller {
	...
	public function auth($action){
		...
		if(empty($post) && $action == 'login'){
		
		     $memberlogin = $this->session->flashdata('memberlogin');
				...
				if($this->authentication->login($username, $password)){
					...
					if(!empty($user_details_array['username']) && sha1($password)==$user_details_array['password']){
						if($user_details_array['status']){
							...
							}else if($user_details_array['type'] == 'designer'){
								$this->user->update_user_login($user_details_array['id']);
								$this->session->set_userdata(array('designer'=>$user_details_array));
								$this->session->set_userdata("login","OK");
								$json['redirect'] = base_url('members/designer');

							}else if($user_details_array['type'] == 'tailor'){
							    $this->session->set_userdata("login","OK");
								$json['redirect'] = base_url('members/designer');
								$this->user->update_user_login($user_details_array['id']);
								$this->session->set_userdata(array('tailor'=>$user_details_array));
								$json['redirect'] = base_url('members/designer');
								
								
								
							}else if($user_details_array['type'] == 'model'){
							    
							    $this->session->set_userdata("login","OK");
								$json['redirect'] = base_url('members/designer');
								$this->user->update_user_login($user_details_array['id']);
								$this->session->set_userdata(array('model'=>$user_details_array));
								$json['redirect'] = base_url('members/model');

							}else if($user_details_array['type'] == 'admin'){
		                        $this->user->update_user_login($user_details_array['id']);
		                        $this->session->set_userdata(array('administrator'=>$user_details_array));
		                        $json['redirect'] = base_url('admincontrol/dashboard');
		                    }else if($user_details_array['type'] == 'client2'){
		                        $this->user->update_user_login($user_details_array['id']);
		                        $this->session->set_userdata(array('user'=>$user_details_array));
    ...
    //                     ));
						...
						if(!empty($data) && $user_type == 'user'){
						    ...
						    //Process to next page if success
						    	$checkUsername = $this->db->query("SELECT id FROM users WHERE username like ". $this->db->escape($this->input->post('username',true)) ." ")->row();
            					if($checkUsername > 0){
            					    
            					   $this->session->set_userdata("userregID",$checkUsername->id);
            					   $json['userData'] = $checkUsername->id;
