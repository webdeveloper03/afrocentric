...
class Product_model extends Model{
    ...
    public function getProducts($filter = array()){
        ...
        if(isset($filter['categoryId']) && $filter['categoryId']){
            ...
            $where .= " AND pc.category_id = {$categoryId}";
        }
        if(isset($filter['searchKey']) && $filter['searchKey']){
            $searchKey = $filter['searchKey'];
            $where .= " AND ( p.product_name LIKE '%{$searchKey}%' )";
        }
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS pc.category_id, pc.product_id, p.product_name, p.product_price, p.product_featured_image 
        FROM product_categories pc LEFT JOIN product p ON p.product_id = pc.product_id 
        WHERE {$where} 
        ORDER BY {$order}";
        ...
        foreach ($records as $key => $value) {
            $image=$value->product_featured_image;
            $records[$key]->product_featured_image_url =  base_url($image != '' ? 'assets/images/product/upload/thumb/' . $image : 'assets/images/no_image_available.png');
        }
